L.Control.Zoomslider = L.Control.extend({
	...
	onAdd: function (map) {
		...
		this._map
			.on('layeradd', this._updateSliderHeight, this)
			.on('zoomend layeradd', this._snapToSliderValue, this);
	...
	_createSlider: function (className, container) {
		this._wrapper =  L.DomUtil.create('div', className + '-wrap', container);
		this._updateSliderHeight();

		var slider = L.DomUtil.create('div', className, this._wrapper);
